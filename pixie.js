...
function avatar(size, dark) {
  var s = size || 38, g = dark ? 'd' : '';
  return '<svg viewBox="0 0 48 48" width="' + s + '" height="' + s + '" xmlns="http://www.w3.org/2000/svg">'
  + '<defs><linearGradient id="pxg' + g + '" x1="0" y1="0" x2="1" y2="1">'
  + '<stop offset="0" stop-color="' + (dark ? '#e8873a' : '#fff') + '"/>'
  + '<stop offset="1" stop-color="' + (dark ? '#c25f1c' : '#ffe6cf') + '"/></linearGradient></defs>'
  + '<rect x="5" y="5" width="12" height="12" rx="2.6" fill="url(#pxg' + g + ')"/>'
  + '<rect x="31" y="5" width="12" height="12" rx="2.6" fill="url(#pxg' + g + ')" opacity=".55"/>'
  + '<rect x="18" y="18" width="12" height="12" rx="2.6" fill="url(#pxg' + g + ')"/>'
  + '<rect x="5" y="31" width="12" height="12" rx="2.6" fill="url(#pxg' + g + ')" opacity=".55"/>'
  + '<rect x="31" y="31" width="12" height="12" rx="2.6" fill="url(#pxg' + g + ')"/>'
  + '<circle cx="10.5" cy="10.5" r="2.4" fill="#12100e"/>'
  + '<circle cx="36.5" cy="10.5" r="2.4" fill="#12100e" opacity=".8"/></svg>';
}
...
/* ══════════ CONNAISSANCES ══════════ */
var KB = [
{k:['delai','delais','combien de temps','quand','recevoir','rapide','expedition','expedie','temps de production'],s:'both',
...
{k:['fichier','pdf','format','resolution','dpi','cmjn','fond perdu','fonds perdus','preparer','jpg','png','poids fichier','mo'],s:'both',
...
{k:['prix','tarif','cout','combien ca coute','cher','budget','pas cher','moins cher','economique'],s:'both',
...
{k:['finition','pelliculage','mat','brillant','soft touch','vernis','dorure','coins arrondis','arrondi','luxe','haut de gamme','qualite'],s:'both',
...
{k:['paiement','payer','bancontact','carte','virement','ideal','kbc','belfius','moyen de paiement','paypal'],s:'both',
...
{k:['livraison','livrer','pays','belgique','pays-bas','luxembourg','benelux','frais de port','port','franco','domicile'],s:'both',
...
{k:['suivi','ou est ma commande','suivre ma commande','statut','tracking','colis','avancement','ou en est','commande en cours'],s:'shop',
...
{k:['panier','commander','comment commander','comment je commande','passer commande','comment ca marche','etapes','processus'],s:'shop',
...
{k:['urgent','rapidement','vite','express','presse','pour demain','au plus vite','delai court','deadline'],s:'both',
...
{k:['contact','contacter','vous contacter','joindre','vous joindre','telephone','appeler','vous appeler','numero de telephone','email','mail','rendez-vous','rdv','parler a quelqu un','parler a un humain','humain','conseiller','un vrai conseiller'],s:'both',
...
/* Fiches produits */
{k:['carte de visite','cartes de visite','carte visite','cdv'],s:'shop',
...
function score(q, keys) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (!kw) continue;
    if (n === kw) sc += 12;
    else if (n.indexOf(kw) >= 0) sc += kw.indexOf(' ') >= 0 ? 7 : 4;
    else if (kw.length > 4) {
      var w = n.split(' ');
      for (var j = 0; j < w.length; j++) {
        if (w[j].length > 3 && (w[j].indexOf(kw) === 0 || kw.indexOf(w[j]) === 0)) { sc += 2; break; }
      }
    }
  }
  return sc;
...
/* ══════════ INTERFACE ══════════ */
var win, body, input, opened = false, greeted = false, turns = 0, lastTopic = null;
...
function productCard(p) {
  ...
  c.innerHTML = '<span class="e">' + esc(p.ico) + '</span><span class="t"><b>' + esc(p.name) +
    '</b><i>' + esc(p.fmt) + '</i></span><span class="p">' + esc(price) + '</span>';
...
function lookupOrder(ref) {
  ...
  setTimeout(function () {
    ...
    supa.from('orders').select('data').eq('ref', ref).limit(1).then(function (r) {
      ...
      if (!row || !row.data) {
        say('Je ne trouve pas <b>' + esc(ref) + '</b> sur votre compte 🤔\n\nVérifiez la référence dans votre e-mail de confirmation, ou écrivez-nous à ' + MAIL + '.');
        return;
      }
      var o = row.data;
      var items = (o.items || []).map(function (i) { return '• ' + esc(i.name) + ' — ' + esc(i.conf); }).join('\n');
      say('Voici votre commande <b>' + esc(o.ref) + '</b> :\n\n' + items +
        '\n\nStatut : <b>' + esc(o.status || '—') + '</b>\nPassée le ' + esc(o.date || '—') +
        '\nLivraison estimée : <b>' + esc(o.deliv || '—') + '</b>' +
        (o.tracking ? '\nSuivi colis : <b>' + esc(o.tracking) + '</b>' : '') +
...
/* Repli intelligent : propose les pistes les plus proches */
function fallback(q) {
  var scored = [];
  for (var i = 0; i < KB.length; i++) {
    var e = KB[i];
  ...
  if (scored.length) {
    say('Je ne suis pas certaine d\'avoir bien saisi 🤔 Vous vouliez parler de :');
    quick(scored.slice(0, 3).map(function (x) { return [x[1].k[0].charAt(0).toUpperCase() + x[1].k[0].slice(1), x[1].k[0]]; })
      .concat([['Aucun des deux', '__contact']]));
    return;
  }
  say('Alors là, vous me prenez de court 🙂\n\nJe ne connais que Pixel Studio — l\'impression, le design, les délais, les commandes. Pour tout le reste, Logan sera bien meilleur que moi :\n\n• <b>' + MAIL + '</b>\n• <b>' + TEL + '</b>');
  quick([['J\'ai un projet à définir', '__besoin'], ['Les questions fréquentes', '__faq']]);
}
...
function route(q, label) {
  if (q === '__besoin') { say(esc(label || 'J\'ai un projet'), 'u'); askNeed(); return; }
  if (q === '__contact') { say(esc(label || 'Parler à quelqu\'un'), 'u'); handle('contact'); return; }
  if (q === '__faq') { say(esc(label || 'Questions fréquentes'), 'u'); faq(); return; }
  if (FLOWS[q]) { say(esc(label || q), 'u'); startFlow(q); return; }
  handle(q, label);
}

function handle(q, label) {
  turns++;
  if (label !== false) say(esc(label || q), 'u');
  ...
  setTimeout(function () {
    ...
    if (e.f) setTimeout(function () { say(e.f); }, 620);
