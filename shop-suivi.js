...
function orderCard(o, isAdmin) {
  ...
  const who = (isAdmin && o.cust)
    ? `<div style="font-size:12.5px;color:var(--muted);margin-top:10px;padding:12px;background:rgba(0,0,0,.25);border-radius:10px">
        ...
        &#129534; ${o.cust.street} ${o.cust.num}, ${o.cust.zip} ${o.cust.city}
        ${o.cust.notes ? '<br>&#128221; ' + o.cust.notes : ''}
       </div>` : '';
  const shipBlock = sp
    ? `<div class="tl-track" style="margin-top:10px">&#128666; <b>Livraison à une autre adresse</b><br>
        ${sp.name}${sp.company ? ' — ' + sp.company : ''}<br>
        ${sp.street} ${sp.num}, ${sp.zip} ${sp.city} (${sp.country || o.country})
        ${sp.phone ? '<br>&#128222; ' + sp.phone : ''}</div>`
    : '';
  ...
  const docs = `<div class="oactions">
      <button class="btn-ghost" onclick="openInvoice('${o.ref}')">&#129534; Facture</button>
      ${isPro ? `<button class="btn-ghost" onclick="downloadPeppol('${o.ref}')">&#128228; Peppol (XML)</button>` : ''}
      ${isAdmin ? `<button class="btn-ghost" onclick="supplierSheet('${o.ref}')">&#127981; Bon fournisseur</button>` : ''}
      ${!isAdmin ? `<button class="btn-ghost" onclick="reorder('${o.ref}')">&#128257; Commander à nouveau</button>` : ''}
    </div>`;
  ...
  const recap = `<div class="orecap">
      ...
      ${o.invoice ? `<div class="orow"><span>N° de facture</span><span>${o.invoice}</span></div>` : ''}
    </div>`;
...
window.supplierSheet=function(ref){
  ...
  const links=(o.fileLinks||[]).map(function(f){
    return '<div><a href="'+f.url+'">'+esc(f.label)+'</a></div>';}).join('')||'<i>Aucun lien</i>';
  const w=window.open('','_blank');
